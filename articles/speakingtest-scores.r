---
title: "【R言語】GASより印刷がラク！スピーキング評価のPDF一括生成ツール"
emoji: "🖨️"
type: "tech"
...
# 【R言語】スピーキング・リテリング評価の個人別フィードバックシートをPDFで一括生成する
...
今回は、Rを使って**学生全員分のレポートを1つのPDFファイルにまとめる**ツールを公開します。

## 1. コードとサンプルデータ（GitHub）

スクリプトと、動作確認用のサンプルデータはGitHubに公開しています。

👉 **[ここにあなたのGitHubリポジトリURLを貼る]**

## 2. 準備するデータ構造

以下のようなCSVファイル（UTF-8形式）を用意します。今回のスクリプトは、1列目が「名前」、数値列が「評価項目」、列名に「コメント」を含む列が「フィードバック」として読み込まれる設計です。
...
[ここにあなたの考察：診断的言語テストにおいて、なぜこれらの観点を可視化することが学習者の気づきに繋がるのか、専門的な視点を記述してください]

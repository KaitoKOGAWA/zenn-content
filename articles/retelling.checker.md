# 【SLA×データ分析】英語教員向け：リテリングの発話をとおして、単語の定着度を「上位・中位・下位」で可視化するブラウザツール

## 結論：この記事で得られること
リテリング（再話）活動において、「どの単語が、どの学力層の生徒に定着しているか」を瞬時に可視化する分析ツールを開発・公開しました。
本記事では、SLA（第二言語習得研究）に基づく評価の考え方と、プログラミング知識ゼロですぐに現場で使える本ツールの利用手順を解説します。

## 1. なぜ「学力層別」の分析が必要なのか？（SLAの視点）

現場のリテリング評価において、「クラス全体の平均点」だけを見ていても、指導の具体的な改善点は見えてきません。診断的言語テストの観点から、以下の理由により学力層別の分析が不可欠です。

* **（※ここに、SLAの知見に基づき「なぜ上位・中位・下位で分けて語彙の使用率を見るべきか」を2〜3文で論理的に記述してください）**
* **（※例：特定の単語が上位層のみで使われている場合、概念的理解に認知負荷がかかっている可能性があり、全体指導の対象とするかどうかの判断基準になる、など）**
...
3. **GitHub経由で即利用可能**
   環境構築は不要です。以下のリンクからすぐにブラウザ上でツールを使用できます。
...
* **[ここにGitHub PagesのURLを記載（例：https://your-username.github.io/retelling-checker/）]**
